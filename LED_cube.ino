...
void setup() {
  ...
  // set up Timer 1
  // Timer0 is reserved for millis() so don't touch it!!
  ...
  OCR1C = 49;                                                      // set compare time to ~200us -> 1/(1e6 / 64 / (49 + 1)) = 400us
...
ISR(TIMER1_COMPA_vect) {
  ...
  if (current_state != button_state) {
    ...
    if (count >= 4) {
      button_state = current_state;  // The button has not bounced for four checks, change state
      if (current_state != 0) {
